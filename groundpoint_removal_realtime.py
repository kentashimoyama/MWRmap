...
## リアルタイム処理
extracted_points = []
def a_time(input_points, t_th, origin_t):
    t = input_points[0]
    if t - origin_t < t_th:
        extracted_points.append(input_points)
...
# 入力点群
#入力点群をROSから取得 input_points

# 積分時間
#積分時間を指定
extracted_points = a_time(input_points, t_th, origin_t) #1フレームの点群、閾値、基準になる時刻
###
