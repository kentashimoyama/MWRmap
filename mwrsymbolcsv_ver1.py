...
iwrdirname = "csv_0708_2_left"
aqlocfilename = "20240708_2_latlon19.csv"
finalfilename = "symbols_0708_2_left.csv"
...
IWR_DIR = r"C:\Users\kenta shimoyama\Documents\amanolab\melco\簡易地図生成\mwr/"+iwrdirname
AQLOC_FILE = r'C:\Users\kenta shimoyama\Documents\amanolab\melco\簡易地図生成\aqloc/'+aqlocfilename
        ...
        if len(timerow) != 3:
            ...
            with open(filepath, "r", encoding="utf-8") as csvfile:
                ...
                for row in csvreader:
                    ...
                    # iwrx = px - (qx+0.3) * cos_theta + (qy+0.07) * sin_theta #調整をかける、right
                    # iwry = py + (qx+0.3) * sin_theta + (qy+0.07) * cos_theta #調整をかける, right

                    #斜め45deg取付の場合
                    sin_theta = math.cos(theta - math.pi / 4)
                    cos_theta = math.sin(theta - math.pi / 4)
                    
                    iwrx = px - (qx+0.48) * cos_theta + (qy+0.03) * sin_theta #調整をかける, left
                    iwry = py + (qx+0.48) * sin_theta + (qy+0.03) * cos_theta #調整をかける, left
                    ...
                    data_list.append([iwrx, iwry, amplitude, timestanp])
...
csv_file = r"C:\Users\kenta shimoyama\Documents\amanolab\melco\簡易地図生成\mwr+aqloc/"+finalfilename
with open(csv_file, 'a', newline='') as file:
    writer = csv.writer(file)
    for i in range(len(x_list)):
        writer.writerow([x_list[i], y_list[i], amplitude_list[i], timestanp_list[i]])