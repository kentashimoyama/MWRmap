...
class GroundPointRemoval():
    def __init__(self):
        # self.th = 5
        # self.voxel_size = 0.36
        sc = Set_config()
        self.home_dir = sc.home_dir
        self.voxel_size = sc.voxel_size
        self.th = sc.gpth

    
    def filter_ground_and_vegetation(self, points, voxel_size=0.05, threshold=5): 
        # グリッドのサイズを設定
        ...
        if len(list(filtered_points)) != 0:
            filtered_points = filtered_points[filtered_points[:, 3].argsort()] #unixtimeの列を基準にソート

        # 直線を描画 open3dを使用。
        #line_set = self.grid_line(grid_min_x, grid_min_y ,grid_max_x, grid_max_y, grid_num_x, grid_num_y, grid_size)
    ...
    def main(self, dirname, visualize):
        ...
        for filename in filename_list:
            ...
            # 点群のフィルタリング
            th = self.th #5
            voxel_size = self.voxel_size #0.36
            filtered_points = self.filter_ground_and_vegetation(world_pos, voxel_size=voxel_size, threshold=th) #gridあたり何点以上
            ...
            output_filename = output_directory + filename.split(".")[0]+"_extracted"+str(int(voxel_size*100))+"_"+str(th)+".csv"
            os.makedirs(output_directory, exist_ok=True)
            extracted_mwr_df.to_csv(output_filename , index=False, header=False)
...
def grid_line(self, grid_min_x, grid_min_y ,grid_max_x, grid_max_y, grid_num_x, grid_num_y, grid_size):
    # 直線の端点
...
if __name__ == "__main__":
    ...
    for csv_dirname in csv_dirname_list:
        ...
        # th = 5
        # voxel_size = 0.36
        # filtered_points = filter_ground_and_vegetation(world_pos, voxel_size=voxel_size, threshold=th) #gridあたり何点以上
