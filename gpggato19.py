...
def convert_gpgga_to_jgd2011(latitude, longitude):
...
def cal_deg(lat, lon, pre_lat, pre_lon, delta_t):
        #if t_pre == 0:
            #return
...
def compute_yaw_angle(vector1, vector2):
    """
    2つのベクトル間のヨー角を計算する
    """
...
def cal_deg2(pro_lat, pro_lon, lat, lon, pre_lat, pre_lon, pro_delta_t, delta_t):
    """
    ...
    for i, row in enumerate(rows):
        if "GPGGA" in rows[i][0]:
            if i > 0:
                ...
                lat, lon = convert_gpgga_to_jgd2011(latitude, longitude)
                if i == 1:
                    row = [time, lat, lon]
                    writer.writerow(row)
                    continue
                ...
                pre_lat, pre_lon = convert_gpgga_to_jgd2011(pre_latitude, pre_longitude)
                pro_lat, pro_lon = convert_gpgga_to_jgd2011(pro_latitude, pro_longitude)

                delta_t = float(time)-float(pre_time)
                pro_delta_t = float(pro_time) - float(time)
                #yaw = cal_deg(lat, lon, pre_lat, pre_lon, delta_t)
                yaw = cal_deg2(pro_lat, pro_lon, lat, lon, pre_lat, pre_lon, pro_delta_t, delta_t)
                
                row = [time, lat, lon, yaw]
                writer.writerow(row)

        # #yaw from IMU inside aqloc
