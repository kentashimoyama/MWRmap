...
def amp_azimath_th(file, th):
    ...
    inlier_ampazi_df = inlier_ampazi_df.sort_values(by=inlier_ampazi_df.columns[3])
...
if __name__ == '__main__':
    cosfitting = True
    MELCOdir = r"C:\Users\kenta shimoyama\Documents\amanolab\melco\簡易地図生成\mwr/"
    file_list = ["20240717_1721193682_adv_111_right.csv", "20240717_1721193686_adv_111_left.csv", "20240717_1721193904_adv_222_left.csv", "20240717_1721193906_adv_222_right.csv"]
    for file in file_list:
        ...
        if cosfitting:
            ...
            output_folder = r"C:\Users\kenta shimoyama\Documents\amanolab\melco\簡易地図生成\mwr\fig/" + str(datetime.datetime.now().strftime('%Y%m%d%H%M%S'))
            fig_output_folder = r"C:\Users\kenta shimoyama\Documents\amanolab\melco\簡易地図生成\mwr\fit_best/" + str(datetime.datetime.now().strftime('%Y%m%d%H%M%S'))
            ...
            if is_MELCO:
                ...
                for i, MWR_time in enumerate(MWR_dict.keys()):
                    ...
                    if os.path.exists(outcsv_name):
                        ...
                        if inlier_df is not None:
                            inlier_df.to_csv(outcsv_name, mode='w', header=True, index=False)
                            inlier_df = None
